...
async function handleGetAICorrelationInterpretation() {
    ...
    try {
        ...
        if (result.candidates && result.candidates.length > 0 &&
            ...
            correlationInsightsText.textContent = "No insights could be generated for this correlation matrix.";
...
async function handleGetAILinearRegressionInterpretation() {
    ...
    try {
        ...
        if (result.candidates && result.candidates.length > 0 &&
            ...
            linearRegressionInsightsText.textContent = "No insights could be generated for these linear regression results.";
            console.warn("Unexpected API response structure:", result);
        }
    } catch (error) {
...
async function handleGetAIRandomForestInterpretation() {
    ...
    try {
        ...
        if (result.candidates && result.candidates.length > 0 &&
            ...
            randomForestInsightsText.textContent = "No insights could be generated for these Random Forest results.";
            console.warn("Unexpected API response structure:", result);
        }
    } catch (error) {
